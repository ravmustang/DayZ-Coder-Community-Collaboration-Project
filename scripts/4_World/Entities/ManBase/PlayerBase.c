...
//ref VehicleManager                      m_VehicleManager;
    ...
    private string                      m_LastHitNote;
    private string                      m_LastHitWeapon;
    private string                      m_LastHitByPlayerName;
    ...
    override void EEKilled(Object killer) {        
        ...
        if (killer.IsMan()) {
            ...
            string killerWeap = sbKiller.GetHumanInventory().GetEntityInHands().GetDisplayName();
            float range = vector.Distance(killer.GetPosition(), this.GetPosition());
            range = Math.Round(range);
            if (sbKiller == sbKilled) {
                if (m_LastHitNote != "Self") {
                    Print("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " died from not attending their wounds.");
                    GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " died from not attending their wounds.");
                } else {
                    Print("[ButlerZ]: The mental health of " + sbKilled.GetPlayerIdentityName() + " took a downturn. Ending their struggle, they fall lifeless to the floor.");
                    GetGame().ChatPlayer(1, "The mental health of " + sbKilled.GetPlayerIdentityName() + " took a downturn. Ending their struggle, they fall lifeless to the floor.");
                }
            } else {
                if (sbKiller == killer) {
                    Print ("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " was beaten to death by " + sbKiller.GetPlayerIdentityName() + " with their bare fists.");
                    GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " was beaten to death by " + sbKiller.GetPlayerIdentityName() + " with their bare fists.");
                } else {
                    Print("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " was shot dead by " + sbKiller.GetPlayerIdentityName() + " from " + range + "m away with a(n) " + killerWeap + ".");
                    GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " was shot dead by " + sbKiller.GetPlayerIdentityName() + " from " + range + "m away with a(n) " + killerWeap + ".");
                }
            }
        } else {
            if (m_LastHitNote == "Self") {
                Print("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " died from effects of self-harm.");
                GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " died from effects of self-harm.");
            } else if (m_LastHitNote == "Infected") {
                Print("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " was eaten alive by the Infected.");
                GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " was eaten alive by the Infected.");
            } else if (m_LastHitNote == "Wolf") {
                Print("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " was mauled by Wolves.");
                GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " was mauled by Wolves.");
            } else if (m_LastHitNote == "Player") {
                if (m_LastHitWeapon != "") {
                    if (m_LastHitWeapon != killer) {
                        Print("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " was killed by " + m_LastHitByPlayerName + " as result of melee with a(n) " + m_LastHitWeapon + ".");
                        GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " was killed by " + m_LastHitByPlayerName + " as result of melee with a(n) " + m_LastHitWeapon + ".");
                    } else {
                        Print ("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " was beaten to death by " + sbKiller.GetPlayerIdentityName() + " with their bare fists.");
                        GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " was beaten to death by " + sbKiller.GetPlayerIdentityName() + " with their bare fists.");
                    }
                } else {
                    Print("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " was killed by " + m_LastHitByPlayerName + " and the effects of their bare knuckles.");
                    GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " was killed by " + m_LastHitByPlayerName + " and the effects of their bare knuckles.");
                }
            } else {
                Print("[ButlerZ]: " + sbKilled.GetPlayerIdentityName() + " was killed by Mysterious Forces.");
                GetGame().ChatPlayer(1, sbKilled.GetPlayerIdentityName() + " was killed by Mysterious Forces.");
            }
        }
    }
    ...
    override void EEHitBy(TotalDamageResult damageResult, int damageType, EntityAI source, string component, string ammo, vector modelPos) {
        ...
        if (damageResult != null) {
            SurvivorBase sbVictim = SurvivorBase.Cast(this);
            if (source == this) {
                Print("[ButlerZ]: " + sbVictim.GetPlayerIdentityName() + " has hurt themselves");
                m_LastHitNote = "Self";
                m_LastHitByPlayerName = "";
                m_LastHitWeapon = "";
            } else if (source != this && source.IsMan()) {
                Print("[ButlerZ]: " + sbVictim.GetPlayerIdentityName() + " was hit by another player (bare fists)");
                SurvivorBase sbA = SurvivorBase.Cast(source);
                m_LastHitNote = "Player";
                m_LastHitByPlayerName = sbA.GetPlayerIdentityName();
                m_LastHitWeapon = "";
            } else if (source.GetDisplayName() == "INFECTED") {
                Print("[ButlerZ]: " + sbVictim.GetPlayerIdentityName() + " was hit by an Infected");
                m_LastHitNote = "Infected";
                m_LastHitByPlayerName = "";
                m_LastHitWeapon = "";
            } else if (source.GetDisplayName() == "Canis Lupus Grey" || source.GetDisplayName() == "Canis Lupus White") {
                Print("[ButlerZ]: " + sbVictim.GetPlayerIdentityName() + " was hit by a Wolf");
                m_LastHitNote = "Wolf";
                m_LastHitByPlayerName = "";
                m_LastHitWeapon = "";
            } else if (!source.IsDayZCreature()) {
                Man manAttacker = source.GetHierarchyParent();
                SurvivorBase sbAttacker = SurvivorBase.Cast(manAttacker);
                m_LastHitNote = "Player";
                m_LastHitByPlayerName = sbAttacker.GetPlayerIdentityName();
                m_LastHitWeapon = source.GetDisplayName();
                Print("[ButlerZ]: Player " + sbVictim.GetPlayerIdentityName() + " was hit by " + sbAttacker.GetPlayerIdentityName() + " with " + source.GetDisplayName());
            } else {
                Print("[ButlerZ]: " + sbVictim.GetPlayerIdentityName() + " was damaged by something else");
                m_LastHitNote = "Unknown";
                m_LastHitByPlayerName = "";
                m_LastHitWeapon = "";
            }
        }
        ...
        if(DiagMenu.GetBool(DiagMenuIDs.DM_MELEE_DEBUG_ENABLE)) {
            Print("EEHitBy() | Character " + GetDisplayName() + " hit by " + source.GetDisplayName() + " to " + component);
        }
            ...
            if (attachment_cargo) {
                for (int cgo_i = 0; cgo_i < attachment_cargo.GetItemCount(); cgo_i++) {
                    ...
                    if (cargo_item.IsMagazine()) {
                        ...
                        } else if (weapon_manager.CanLoadBullet(weapon_base, magazine)) { //bullets (get ammo pile with min ammo count)
                            if (mag_ammo_count > 0) {
                                if (last_mag_ammo_count == 0) {
                                    magazine_to_reload = EntityAI.Cast(magazine);
                                    last_mag_ammo_count = mag_ammo_count;
